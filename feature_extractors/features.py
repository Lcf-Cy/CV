...
class Features(torch.nn.Module):
    ...
    def __init__(self, args, image_size=224, f_coreset=0.1, coreset_eps=0.9):
        ...
        self.resize2 = torch.nn.AdaptiveAvgPool2d((28, 28))
    ...
    def __call__(self, rgb, xyz):
        ...
        with torch.no_grad():
            ...
            # x1,x2,x3,x4, xyz_feature_maps, center, ori_idx, center_idx = self.deep_feature_extractor(rgb, xyz)
        '''
        首先，代码定义了一个布尔变量interpolate并将其设置为True。这个变量用来控制是否进行特征插值操作。

        接着，通过条件判断语句if interpolate:来检查interpolate是否为True。如果是，则执行下面的代码块。

        在代码块中，调用了interpolating_points函数，并传入了三个参数：xyz、center.permute(0,2,1)和xyz_feature_maps。
        这个函数的作用是对点云特征进行插值操作，
        生成插值后的特征图interpolated_feature_maps。插值操作的具体过程和方法可以在interpolating_points函数的实现中找到。

        接着，将插值后的特征图interpolated_feature_maps移动到CPU上，即使用.to("cpu")方法将其转移到CPU内存中保存。

        继续执行下面的代码，对xyz_feature_maps和rgb_feature_maps进行处理。首先，将其转换为列表形式，并对每个元素应用.to("cpu")方法，将其移动到CPU上。
        '''
        interpolate = True    
        if interpolate:
        ...
        if interpolate:
            ...
            # return x1,x2,x3,x4, xyz_feature_maps, center, ori_idx, center_idx, interpolated_feature_maps
        else:
            # return x1,x2,x3,x4, xyz_feature_maps, center, ori_idx, center_idx
    ...
    def get_coreset_idx_randomp(self, z_lib, n=1000, eps=0.90, float16=True, force_cpu=False):
        ...
        try:
            ...
            # z_lib = torch.tensor(transformer.fit_transform(z_lib).detach().numpy())
            z_lib = torch.tensor(transformer.fit_transform(z_lib.detach().numpy())).detach()
